                ...
                <div id="questions">
                    @foreach ($baiKiemTra->list_cau_hoi as $index => $item)
                        <div class="question-card" data-question="{{ $index + 1 }}" data-id-question="{{$item->id}}">
                            ...
                            <div>
                                <div><input type="radio" name="q{{ $index + 1 }}" value="a"> {{ $item->dap_an_a }}</div>
                                <div><input type="radio" name="q{{ $index + 1 }}" value="b"> {{ $item->dap_an_b }}</div>
                                <div><input type="radio" name="q{{ $index + 1 }}" value="c"> {{ $item->dap_an_c }}</div>
                                <div><input type="radio" name="q{{ $index + 1 }}" value="d"> {{ $item->dap_an_d }}</div>
                            </div>
                        </div>
                    @endforeach
                </div>
